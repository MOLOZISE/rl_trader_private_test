...
def main(stock_code, model_name, target_date, actor_path=None, value_path=None, num_episodes=100):
    ...
    #    os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format(target_date, stock_code)), ver='y3')
    #    os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format(target_date, stock_code)), ver='y2')
        os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format(target_date, stock_code)), ver='h1')
    age = extreme_agent.Agent(environment=environment.Environment(chart_data, training_data))
...
def relearn(stock_code1, stock_code2, model_name, new_model_name, target_date ,actor_path=None, value_path=None, num_episodes=100):
    ...
    initial_balance = 10000000 # threemin, fivemin
    chart_data, training_data = data_manager.load_data(
    #    os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format(target_date, stock_code1)), ver='y3')
    #    os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format(target_date, stock_code1)), ver='y2')
        os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format(target_date, stock_code1)), ver='h1')
    age = extreme_agent.Agent(environment=environment.Environment(chart_data, training_data), min_trading_unit=1,
                      max_trading_unit=2)
    age.set_balance(initial_balance)
    num_features = age.STATE_DIM
    input_len = age.environment.get_training_data_shape()
...
def day_test(stock_code1, stock_code2, model_name, target_date, actor_path=None, value_path=None, is_random=False):
    ...
    initial_balance = 10000000
    chart_data, training_data = data_manager.load_data(
         os.path.join(settings.BASE_DIR, 'files/{}/{}.txt'.format("testdaysdata", stock_code1)), ver='y1')
    age = extreme_agent.Agent(environment=environment.Environment(chart_data, training_data), min_trading_unit=1,
                      max_trading_unit=2)
    age.set_balance(initial_balance)
    num_features = age.STATE_DIM
...
def autolearning_datever(start_date="20200720", end_date="20200807"):
    ...
    for i in range(delta.days + 1):
        if count < 10:
            ...
            count_str = str(count)
        model_name = "profitother100_total_zero_extreme_h1_5steps_2basic_1000episodes_" + count_str
        #model_name = "memoryzeroonechangeotheradjust_total_simul_extreme_z3_1steps_2basic_1000episodes_" + count_str
        #model_name = "memoryprofitotheradjust100_total_simul_extreme_y2_15steps_2basic_1000episodes_" + count_str
        # simul 해보고 그담에 일반 버전
        ...
        elif model_name == "newprouprate100_eighty_simul_extreme_30min_5steps_" + count_str:
        ...
        elif model_name == "profitotherconse100_total_simul_extreme_y2_5steps_2basic_1000episodes_" + count_str:
            filepath = "files/" + target_date + ".txt"
        elif model_name == "memoryzeroonechangeotheradjust_total_simul_extreme_z3_1steps_2basic_1000episodes_" + count_str:
            filepath = "files/eight_ten_three_month_big.txt"
        elif model_name == "profitother100_total_zero_extreme_h1_5steps_2basic_1000episodes_" + count_str:
            filepath = "files/" + target_date + ".txt"
        else:
            print("error")
            return
...
def autoverifying_datever(start_date="20200803", end_date="20200807"):
    ...
    count = 1
    test_start_date = "20201203"
    test_end_date = "20201207"
    ...
    for i in range(seta.days + 1):
        ...
        temp_filepath = "files/" + target_date + ".txt"
        # 이거 왜함???
        ...
        for j in range(delta.days + 1):
            ...
            # 전체 데이터로 테스트(100개)
            filepath = "files/" + test_target_date + ".txt"
            # 없는 경우 넘어감
            if not os.path.isfile(filepath):
                continue
            # 현재 날짜 적기
...
def recoverlearning_datever(start_date="20200720", end_date="20200807"):
    ...
    count = 14
    before_date = "20201023"
    before_model_name = "memory100_total_simul_extreme_y2_5steps_2basic_1000episodes_14"
    # 각 종목코드별로
    print("before_model_name : %s" % before_model_name)
    for i in range(delta.days + 1):
        if count < 10:
            ...
            count_str = str(count)
        model_name = "memory100_total_simul_extreme_y2_5steps_2basic_1000episodes_" + count_str
        target_date = str(d1 + datetime.timedelta(days=i)).replace('-', '')
        code_list = []
        ...
        if model_name == "profit_twenty" + count_str:
            ...
            filepath = "files/" + target_date + "/highprofit.txt"
        elif model_name == "memory100_total_simul_extreme_y2_5steps_2basic_1000episodes_" + count_str:
        ...
        for code in code_list:
            print("current code : %s count : %s / %s" % (code, pure_count, len(code_list)))
            relearn(code, before_date, before_model_name, model_name, target_date, num_episodes=1000)
            before_date = target_date
            before_model_name = model_name
            pure_count += 1
        count += 1
...
def autolearning_random(start_date="20200720", end_date="20200807"):
    ...
    for episodes in range(num_learning):  # 스타크래프트 2 1백만
        ...
        if (episodes + 1) % save_interval == 0:
            ...
            model_name = "changeotheradjust1000_total_simul_extreme_y3_3steps_2basic_repeat1onemil_" + str(count)
            network_name_actor = model_name + '_actor'
            network_name_value = model_name + '_value'
            path_actor = os.path.join(settings.BASE_DIR, 'models/{}.h5'.format(network_name_actor))
            path_value = os.path.join(settings.BASE_DIR, 'models/{}.h5'.format(network_name_value))
...
def autoverifying_randomver(start_date="20200803", end_date="20200807"):
    ...
    for i in range(seta.days + 1):
        ...
        model_name = "changeminusother10_random_nonetax_extreme_y1_3steps_100episodes_nodone_" + str(counting)
        #############################
        target_date = str(d3 + datetime.timedelta(days=i)).replace('-', '')
        # temp_filepath = "files/selected/" + target_date + ".txt"
        # 없는 경우 넘어감 (txt파일이 있다면 있는 것으로 간주)
...
def autolearning_daysver(start_date="20200720", end_date="20200807"):
    ...
    for code in code_list:
        ...
        model_name = "newdataprofitadjust10_total_simul_extreme_y1_1steps_2basic_10000episodes_" + count_str
...
def autoverifying_daysever(start_date="20200803", end_date="20200807", target_date="", is_random=False):
    ...
    # target_date = ""
    # count_str = "50000"
    # model_name = "changeprofit100_random_zero_extreme_z3_1steps_plus2basic_1episodes_" + count_str
    # target_date = 모델의 날짜 이름
    # temp_filepath = "files/selected/" + target_date + ".txt"
    # 없는 경우 넘어감 (txt파일이 있다면 있는 것으로 간주)
    #temp_filepath = "files/" + target_date + "/testvol.txt"
...
def autolearning_daysver_random(start_date="20200720", end_date="20200807"):
    ...
    for episodes in range(num_learning):  # 스타크래프트 2 1백만
        ...
        if (episodes + 1) % save_interval == 0:
            count = episodes + 1 + relearncount
            #model_name = "changeprofit100_random_simul_extreme_z3_1steps_plus2basic_10episodes_" + str(count) # 3만까지함
...
if __name__ == "__main__":
    ...
    # 5, 15, 30 다 해보고... 상위 20개만 테스트 한 걸로 확인해보기 (adjustversion
    print_current_time()
    # 20201013의 36까지함
    #start_date = "20201203"
    #end_date = "20201207"
    # 5일에 46부터
    # 1007 54부터 하면 됨 chage 20201116
    # training data 선정의 문제점...
    # 얼만큼을
    #otherprofit 2020 1102 기준 14일의 8까지 함(tempcodelist 참고)
    #memory 20201007부터 autolearning
    #autolearning_random(start_date=start_date, end_date=end_date)
    #autolearning_datever(start_date=start_date, end_date=end_date)
    #autolearning_daysver()
    #autolearning_daysver_random()
    #recoverlearning_daysver()
    #autoverifying_daysever(is_random=True)
    #autoverifying_daysever()
    #recoverlearning_datever(start_date=start_date, end_date=end_date)
    test_start_date = "20201208"
    test_end_date = "20201208"
    ...
    # 아직 30step의 verify가 안되어있음
    # 통합적으로 5, 15, 30 verify를 20, 40, 60, 80, 100뭐 이런식으로 확장 검사 가능
