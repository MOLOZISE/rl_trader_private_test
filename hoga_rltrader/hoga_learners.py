...
from hoga_environment import Hoga_Environment
...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None,
        ...
        self.environment = Hoga_Environment(chart_data)
        # 에이전트 설정
    ...
    def fit(self, delayed_reward, discount_factor, full=False):
        ...
        # 배치 학습 데이터 생성 및 신경망 갱신 , 최대로의 보상도 확인
        if batch_size > 0:
    ...
    def run(
        ...
        for epoch in range(num_epoches):
            ...
            while True:
                ...
                # 결정한 행동을 수행하고 즉시 보상과 지연 보상 획득 // act의 변경
                # immediate_reward, delayed_reward = \
                #     self.agent.act(action, confidence)
                immediate_reward, delayed_reward = \
                    self.agent.hoga_act(action, confidence)
                #immediate_reward, delayed_reward = self.agent.advanced_act(action, confidence)

                # 행동 및 행동에 대한 결과를 기억
                self.memory_sample.append(list(q_sample)) # 상태
                self.memory_action.append(action) # 행동
                self.memory_reward.append(immediate_reward) # 보상
                if self.value_network is not None:
                    self.memory_value.append(pred_value) # 가치
                if self.policy_network is not None:
                    self.memory_policy.append(pred_policy) # 행동 확률
                    self.memory_pro_a.append(pred_policy[action])
                self.memory_pv.append(self.agent.portfolio_value) # PV
                self.memory_num_stocks.append(self.agent.num_stocks) # 주식 수
                if exploration:
                    self.memory_exp_idx.append(self.training_data_idx) # 탐험수
                ...
                # 지연 보상이란 세대 전체 학습 전에 임계 손익률을 넘겨서 그 상황을 한번 더 fit하는것
...
class ActorCriticLearner(ReinforcementLearner):
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        for i, (sample, action, value, policy, reward) \
            ...
            #r = (reward_next - reward) * 100
            y_value[i, action] = r + discount_factor * value_max_next
            y_policy[i, action] = new_softmax(value)[action]
            value_max_next = value.max()
            reward_next = reward
...
class A2CLearner(ActorCriticLearner):
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        for i, (sample, action, value, policy, reward) \
            ...
            y_policy[i, action] = new_softmax(advantage)[action]
            value_max_next = value.max()
            reward_next = reward
        return x, y_value, y_policy
...
class PPO(ReinforcementLearner):
    def __init__(self, *args, shared_network=None,
                 value_network_path=None, policy_network_path=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.lmbda = 0.95
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        memory = zip(
            ...
            reversed(self.memory_policy[-batch_size:]),
            reversed(self.memory_reward[-batch_size:]),
            reversed(self.memory_pro_a[-batch_size:])
        )
        x = np.zeros((batch_size, self.num_steps, self.num_features))
        y_value = np.zeros((batch_size, self.agent.NUM_ACTIONS))
        y_policy = np.full((batch_size, self.agent.NUM_ACTIONS), .5)
        value_max_next = 0
        reward_next = self.memory_reward[-1]
        for i, (sample, action, value, policy, reward, pro_a) \
                in enumerate(memory):
            x[i] = sample
            y_value[i] = value
            y_policy[i] = policy
